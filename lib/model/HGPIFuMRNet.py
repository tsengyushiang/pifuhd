...
class HGPIFuMRNet(BasePIFuNet):
    ...
    def filter_global(self, images, imagesback):
        '''
        ...
        if self.opt.train_full_pifu:
            self.netG.filter(images, imagesback)
        else:
            with torch.no_grad():
                self.netG.filter(images, imagesback)
    ...
    def filter_local(self, images, rect=None):
        ...
        if len(nmls):
            ...
            if rect is None:
                images = torch.cat(
                    [images, nmls[:, None].expand(-1, images.size(1), -1, -1, -1)], 2)
            else:
                nml = []
    ...
    def query(self, points, calib_local, calib_global=None, transforms=None, labels=None):
        ...
        for i in range(B):
            ...
            if labels is not None:
                ...
                with torch.no_grad():
                    ws.append(in_bb.size(2) /
                              in_bb.view(in_bb.size(0), -1).sum(1))
                    gammas.append(1 - newlabels[-1].view(newlabels[-1].size(
                        0), -1).sum(1) / in_bb.view(in_bb.size(0), -1).sum(1))
            ...
            for j, im_feat in enumerate(self.im_feat_list):
                point_local_feat_list = [self.index(
                    im_feat.view(-1, B, *im_feat.size()[1:])[:, i], xy), z_feat]
        ...
        # first dim is for intermediate predictions
        self.preds_interm = torch.cat(preds_interm, 1)
        # first dim is for intermediate predictions
        self.preds_low = torch.cat(preds_low, 1)
    ...
    def calc_normal(self, points, calib_local, calib_global, transforms=None, labels=None, delta=0.001, fd_type='forward'):
        ...
        im_feat = self.im_feat_list[-1].view(-1,
                                             B, *self.im_feat_list[-1].size()[1:])
        ...
        for i in range(B):
            ...
            self.netG.query(points=points_all,
                            calibs=calib_global, update_pred=False)
            z_feat = self.netG.phi
            if not self.opt.train_full_pifu:
                z_feat = z_feat.detach()
    ...
    def forward(self, images_local, images_global, points, calib_local, calib_global, labels, points_nml=None, labels_nml=None, rect=None):
        ...
        if points_nml is not None and labels_nml is not None:
            self.calc_normal(points_nml, calib_local,
                             calib_global, labels=labels_nml)
        res = self.get_preds()
